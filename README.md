...
- 肉食
  - 空腹時は草食を探して追跡する
  - 餌不足時に近くの獲物を見失いにくくするため、獲物探索範囲は160
  - 草食を食べるとエネルギー回復
  - 5〜7回程度食べると繁殖状態へ
  - 繁殖成功時は1体の子個体を生成
...
旧ソースから数値を確定できなかったエネルギー量・消費量・通常移動速度・草の自然再生間隔などは `SimulationConfig` にまとめ、あとから容易に調整できるようにしています。草食・肉食の1回あたりの出産数、種別ごとの繁殖相手探索範囲、肉食の獲物探索範囲、`framesPerAge`、種別ごとの寿命範囲、死亡表示時間も `SimulationConfig` で調整できます。
