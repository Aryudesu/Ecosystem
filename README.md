...
旧ソースから数値を確定できなかったエネルギー量・消費量・通常移動速度・草の自然再生間隔などは `SimulationConfig` にまとめ、あとから容易に調整できるようにしています。草食・肉食の1回あたりの出産数に加えて、`framesPerAge` と種別ごとの寿命範囲も `SimulationConfig` で調整できます。
