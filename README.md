...
- 個体状態
  ...
  - Breeding
- 草食
  - 肉食が32以内にいる場合は、肉食の状態に関係なく緊急逃走する
  - 32〜64以内では、空腹中の肉食だけを脅威として逃走する
  - 通常・繁殖中の肉食が32より遠い場合は、近くにいても採食や繁殖を中断しない
  ...
  - 繁殖成功時の子個体数は現在の草食個体数に応じて変化する
    ...
    - 100〜199: 1〜2体
    - 200以上: 1体
  - 個体上限で1体も出生できなかった場合も、その繁殖機会は消費した扱いにして繁殖待ち個体の蓄積を抑える
  - 繁殖相手の探索範囲は64
  - 繁殖までに十分な採食機会を得られるよう、エネルギー消費量は0.10 / simulation step
- 肉食
  - 空腹時は草食を探して追跡する
  - 草食が十分いるときは、獲物探索範囲を空腹度に応じて段階的に拡大する
    - 通常の空腹時: 64
    - energy <= 30: 128
    - energy <= 15: 160
  - 草食個体数が減ると、最後の少数個体を飢えた肉食が広範囲から一掃しないよう探索強化を抑える
    - 草食100以上: 64 / 128 / 160
    - 草食50〜99: 最大128（64 / 128）
    - 草食50未満: 64固定
  - 草食を食べるとエネルギー回復
  - 5〜7回程度食べると繁殖状態へ
  - 繁殖成功時は1体の子個体を生成
  - 繁殖で生まれた個体の初期エネルギーは60
  - 繁殖成功時は親から60エネルギーを消費し、親から子へエネルギーを移す形にする
  - 初期配置の肉食は従来どおり最大エネルギー130で開始する
...
旧ソースから数値を確定できなかったエネルギー量・消費量・通常移動速度・草の自然再生間隔などは `SimulationConfig` にまとめ、あとから容易に調整できるようにしています。草食の緊急逃走距離、草食の密度依存繁殖の閾値・段階別出産数、肉食の出産数・出生時エネルギー・繁殖時エネルギー消費、種別ごとの繁殖相手探索範囲、肉食の空腹度別獲物探索範囲・草食個体数による探索制限閾値、草の局所密度半径・上限・自然再生試行回数、`framesPerAge`、種別ごとの寿命範囲、死亡表示時間も `SimulationConfig` で調整できます。
