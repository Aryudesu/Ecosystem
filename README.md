...
- 草食
  - 近くの肉食から逃げる
  - 逃走方向がフィールド外へ向く場合は、壁に押し付けられ続けないよう壁沿いへスライドする
  - 空腹時は草を探す
  - 草を食べるとエネルギー回復
  - 2〜4回程度食べると繁殖状態へ
  - 繁殖成功時は1〜2体の子個体を生成
...
旧ソースから数値を確定できなかったエネルギー量・消費量・通常移動速度・草の自然再生間隔などは `SimulationConfig` にまとめ、あとから容易に調整できるようにしています。草食・肉食の1回あたりの出産数、種別ごとの繁殖相手探索範囲、肉食の空腹度別獲物探索範囲、`framesPerAge`、種別ごとの寿命範囲、死亡表示時間も `SimulationConfig` で調整できます。
