...
void Simulation::Reset() {
    ...
    for (int i = 0; i < config_.initialHerbivores; ++i) TrySpawnAnimal(Species::Herbivore, RandomPosition());
    for (int i = 0; i < config_.initialCarnivores; ++i) TrySpawnAnimal(Species::Carnivore, RandomPosition());
    for (int i = 0; i < config_.initialGrass; ++i) TrySpawnGrass(RandomPosition());
}
...
void Simulation::Update() {
    ...
    for (std::size_t i = 0; i < animals_.size(); ++i) {
        ...
        UpdateState(animal);
        if (animal.species == Species::Herbivore) UpdateHerbivore(i);
        else UpdateCarnivore(i);
    }
    UpdateGrass();
    TryRegrowGrass();
}
...
void Simulation::UpdateVisualEffects() {
    for (auto& death : deathEffects_) {
        ...
        if (death.remainingFrames <= 0) death = {};
    }
}

void Simulation::UpdateState(Animal& animal) {
    if (!animal.active) return;
    const float hungryEnergy = animal.species == Species::Herbivore ? config_.herbivoreHungryEnergy : config_.carnivoreHungryEnergy;
    if (animal.energy <= hungryEnergy) {
        animal.state = LifeState::Hungry;
        return;
    }
    animal.state = animal.meals >= animal.breedTarget ? LifeState::Breeding : LifeState::Normal;
}

bool Simulation::AdvanceAge(Animal& animal) {
    if (!animal.active || config_.framesPerAge <= 0 || animal.lifespan <= 0) return false;
    ++animal.ageFrames;
    if (animal.ageFrames < config_.framesPerAge) return false;
    animal.ageFrames = 0;
    ++animal.age;
    if (animal.age < animal.lifespan) return false;
...
void Simulation::UpdateHerbivore(std::size_t index) {
    ...
    const int predatorIndex = emergencyPredatorIndex >= 0 ? emergencyPredatorIndex : hungryPredatorIndex;
    if (predatorIndex >= 0) {
    ...
    if (animal.state == LifeState::Hungry) {
        ...
        if (grassIndex >= 0) {
            ...
            if (DistanceSquared(animal.position, food.position) <= config_.interactionRadius * config_.interactionRadius) {
                ...
                ++animal.meals;
                if (animal.meals >= animal.breedTarget) animal.state = LifeState::Breeding;
            } else {
                animal.motion = AnimalMotion::Run;
                MoveToward(animal, food.position, config_.herbivoreFoodSpeed);
            }
        } else Wander(animal, config_.herbivoreWanderSpeed);
    } else Wander(animal, config_.herbivoreWanderSpeed);
    ClampToWorld(animal);
}
...
void Simulation::UpdateCarnivore(std::size_t index) {
    ...
    if (animal.state == LifeState::Hungry) {
        ...
        for (const auto& candidate : animals_) if (candidate.active && candidate.species == Species::Herbivore) ++herbivorePopulation;
        const int baseOnlyThreshold = std::max(0, std::min(config_.carnivoreBaseOnlyPreySensePopulation, config_.carnivoreReducedPreySensePopulation));
        const int reducedSenseThreshold = std::max(baseOnlyThreshold, std::max(config_.carnivoreBaseOnlyPreySensePopulation, config_.carnivoreReducedPreySensePopulation));
        ...
        if (herbivorePopulation < baseOnlyThreshold) {
            ...
            if (animal.energy <= config_.carnivoreDesperateEnergy) preySenseRadius = config_.carnivoreDesperatePreySenseRadius;
        ...
        } else Wander(animal, config_.carnivoreWanderSpeed);
    } else Wander(animal, config_.carnivoreWanderSpeed);
    ClampToWorld(animal);
}
...
void Simulation::MoveAway(Animal& animal, const Vec2& target, float speed) {
    ...
    for (const auto& candidate : animals_) {
        if (candidate.active && candidate.species == Species::Carnivore) { hasPredator = true; break; }
    }
    if (!hasPredator) {
    ...
    for (int directionIndex = 0; directionIndex < EscapeDirectionCount; ++directionIndex) {
        ...
        const bool betterNearest = nearestPredatorDistance > bestNearestPredatorDistance + ScoreEpsilon;
        const bool sameNearest = std::abs(nearestPredatorDistance - bestNearestPredatorDistance) <= ScoreEpsilon;
...
void Simulation::ClampToWorld(Animal& animal) {
    ...
    if (animal.position.x != oldX || animal.position.y != oldY) animal.wanderDirection = RandomDirection();
}

void Simulation::UpdateFacing(Animal& animal, float deltaX) {
    constexpr float Epsilon = 0.0001f;
    if (deltaX < -Epsilon) animal.facingLeft = true;
    else if (deltaX > Epsilon) animal.facingLeft = false;
}
...
int Simulation::CountGrassNear(const Vec2& position, float radius) const {
    ...
    int count = 0;
    for (const auto& grass : grass_) if (grass.active && DistanceSquared(position, grass.position) <= limit) ++count;
    return count;
}

int Simulation::RandomHerbivoreBreedTarget() {
    const int population = GetPopulation().herbivores;
    int minMeals = config_.herbivoreBreedMealsMin5;
    int maxMeals = config_.herbivoreBreedMealsMax5;
    if (population < config_.herbivoreBreedMealsThreshold1) {
        minMeals = config_.herbivoreBreedMealsMin1;
        maxMeals = config_.herbivoreBreedMealsMax1;
    } else if (population < config_.herbivoreBreedMealsThreshold2) {
        minMeals = config_.herbivoreBreedMealsMin2;
        maxMeals = config_.herbivoreBreedMealsMax2;
    } else if (population < config_.herbivoreBreedMealsThreshold3) {
        minMeals = config_.herbivoreBreedMealsMin3;
        maxMeals = config_.herbivoreBreedMealsMax3;
    } else if (population < config_.herbivoreBreedMealsThreshold4) {
        minMeals = config_.herbivoreBreedMealsMin4;
        maxMeals = config_.herbivoreBreedMealsMax4;
    }
    const int safeMin = std::max(1, std::min(minMeals, maxMeals));
    const int safeMax = std::max(safeMin, std::max(minMeals, maxMeals));
...
bool Simulation::TrySpawnAnimal(Species species, const Vec2& position, float initialEnergy, const AnimalTraits& traits) {
    ...
    if (speciesCount >= speciesLimit) return false;
    for (auto& animal : animals_) {
        ...
        if (species == Species::Herbivore) {
            animal.breedTarget = RandomHerbivoreBreedTarget();
        } else {
            const int minBreedMeals = std::max(1, std::min(config_.carnivoreBreedMealsMin, config_.carnivoreBreedMealsMax));
            const int maxBreedMeals = std::max(minBreedMeals, std::max(config_.carnivoreBreedMealsMin, config_.carnivoreBreedMealsMax));
            animal.breedTarget = random_.Int(minBreedMeals, maxBreedMeals);
        }
        animal.age = 0;
        animal.ageFrames = 0;
        const int configuredMinLifespan = species == Species::Herbivore ? config_.herbivoreLifespanMin : config_.carnivoreLifespanMin;
...
void Simulation::SpawnGrassAround(const Vec2& position, int count) {
    ...
    for (int i = 0; i < count; ++i) {
        Vec2 p{ position.x + static_cast<float>(random_.Int(-24, 24)), position.y + static_cast<float>(random_.Int(-24, 24)) };
        ...
        if (CountGrassNear(p, config_.grassLocalDensityRadius) >= localLimit) continue;
...
void Simulation::SpawnDeathEffect(Species species, const Vec2& position) {
    ...
    for (auto& death : deathEffects_) if (!death.active) { slot = &death; break; }
    if (slot == nullptr) {
        slot = &*std::min_element(deathEffects_.begin(), deathEffects_.end(), [](const DeathEffect& lhs, const DeathEffect& rhs) { return lhs.remainingFrames < rhs.remainingFrames; });
    }
    slot->active = true;
...
void Simulation::KillAnimal(Animal& animal, DeathCause cause) {
    ...
    auto& speciesStats = species == Species::Herbivore ? statistics_.herbivore : statistics_.carnivore;
    switch (cause) {
    case DeathCause::OldAge: ++speciesStats.oldAgeDeaths; break;
    case DeathCause::Starvation: ++speciesStats.starvationDeaths; break;
    case DeathCause::Predation: ++speciesStats.predationDeaths; break;
    }
...
void Simulation::TryBreed(std::size_t index) {
    ...
    if (animal.species == Species::Herbivore) {
        ...
        const int lowDensityThreshold = std::min(config_.herbivoreLowDensityThreshold, config_.herbivoreHighDensityThreshold);
        const int highDensityThreshold = std::max(config_.herbivoreLowDensityThreshold, config_.herbivoreHighDensityThreshold);
    ...
    if (spawnedOffspring > 0) {
        auto& speciesStats = animal.species == Species::Herbivore ? statistics_.herbivore : statistics_.carnivore;
        speciesStats.births += static_cast<unsigned long long>(spawnedOffspring);
        if (animal.species == Species::Carnivore) {
            const float breedingCost = std::max(0.0f, config_.carnivoreBreedingEnergyCost) * static_cast<float>(spawnedOffspring);
            animal.energy = std::max(0.0f, animal.energy - breedingCost);
        }
    }
    const bool herbivoreAttemptBlockedByCapacity = animal.species == Species::Herbivore && offspringTarget > 0 && spawnedOffspring == 0;
...
void Simulation::TryRegrowGrass() {
    ...
    if (GetPopulation().grass >= static_cast<int>(SimulationConfig::MaxGrass)) return;
    const int localLimit = std::max(1, config_.grassLocalDensityLimit);
    const int attempts = std::max(1, config_.grassRegrowAttempts);
    for (int attempt = 0; attempt < attempts; ++attempt) {
        const Vec2 position = RandomPosition();
        if (CountGrassNear(position, config_.grassLocalDensityRadius) >= localLimit) continue;
        if (TrySpawnGrass(position)) return;
    }
}
...
Vec2 Simulation::RandomPosition() {
    return { static_cast<float>(random_.Int(0, SimulationConfig::Width - 1)), static_cast<float>(random_.Int(0, SimulationConfig::Height - 1)) };
}
...
Population Simulation::GetPopulation() const {
    ...
    for (const auto& grass : grass_) if (grass.active) ++result.grass;
    return result;
}
...
void Simulation::Draw() const {
    ...
    for (const auto& grass : grass_) {
        ...
        DrawCircle(static_cast<int>(grass.position.x), static_cast<int>(grass.position.y), seed ? 2 : 3, seed ? seedColor : grassColor, TRUE);
    }
    ...
    for (const auto& animal : animals_) {
        ...
        if (animal.state == LifeState::Hungry) DrawCircle(x, y, radius + 2, hungryColor, FALSE, 1);
        else if (animal.state == LifeState::Breeding) DrawCircle(x, y, radius + 2, breedingColor, FALSE, 2);
    }
}
